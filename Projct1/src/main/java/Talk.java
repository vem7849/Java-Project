...
public class Talk {
  ...
  public boolean autoMode(String hostname, int portnumber) {
    if (clientMode(hostname, portnumber)) {
      return true;
    } else if (serverMode(portnumber)) {
      return true;
    }
  ...
  public boolean clientMode(String hostname, int portnumber) {
    try {
      return clientMode(new TalkClient(hostname, portnumber));
    } catch (IOException e) {
  ...
  public boolean serverMode(int portnumber) {
    try {
      return serverMode(new TalkServer(portnumber));
    } catch (IOException e) {
  ...
  public boolean start(String[] args) {
    ...
    try {
      ...
      if (cli.has("-s")) {
        return serverMode(cli.has("-p") ? cli.getInt("-p") : 12987);
      }
      if (cli.has("-h")) {
        return clientMode(cli.get("-h"), cli.has("-p") ? cli.getInt("-p") : 12987);
      }
      if (cli.has("-a")) {
        return autoMode(cli.get("-a"), cli.has("-p") ? cli.getInt("-p") : 12987);
      }
    } catch (Exception e) {
      return false;
    }
